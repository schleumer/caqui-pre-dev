...
class ValuedTabs extends Base {
  ...
  render() {
    ...
    for (const item of this.props.children) {
      ...
      tabs.push((
        <li role="presentation" key={ item.props.id } onClick={ this.selectTab(item.props.id) } className={ className }>
          <a href="javascript:;" aria-controls="home" role="tab" data-toggle="tab" key={ item.props.header }>
            { item.props.header }
          </a>
        </li>
        ));
